...
var HC = (function($) {
    ...
    function toggleQuickview($container, show) {
        var $qv = $container.find('div.quickview');
        $qv.toggle(show);
    }
...
$('body').on('mouseenter', 'li.product_container div.image', function() {
    HC.toggleQuickview($(this), true);
});

$('body').on('mouseleave', 'li.product_container div.image', function() {
    HC.toggleQuickview($(this), false);
});

$('body').on('click', 'div.quickview', function() {
    var link = $(this).closest('div.image').find('a.image-link').attr('href');
    BLC.ajax({ url: link }, function(data) {
        data.find('.product-options').removeClass('hidden');
        data.find('.product-option-nonjs').remove();
        $.modal(data, HC.quickviewOptions);
